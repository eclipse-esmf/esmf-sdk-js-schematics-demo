...
import {BaseConfigFilterService, FilterEnums} from './base-config.filter.service';
...
@Component({
  selector: 'esmf-ui-base-config-v210',
  templateUrl: './base-config.component.html',
  styleUrls: ['./base-config.component.scss'],
})
export class BaseConfigComponent implements OnInit, AfterViewInit, AfterViewChecked, OnDestroy {
  ...
  @ViewChild(BaseConfigConfigMenuComponent) private configurationComponent!: BaseConfigConfigMenuComponent;
  ...
  readonly KEY_LOCAL_STORAGE_BASE_CONFIG_V210_COLUMNS = 'base_config_v210_columns';

  readonly KEY_LOCAL_STORAGE_BASE_CONFIG_V210_CONFIG = 'base_config_v210_config';
  ...
  initializeColumns(): void {
    const configStorage = this.storageService.getItem(this.KEY_LOCAL_STORAGE_BASE_CONFIG_V210_CONFIG);
    const columnStorage = this.storageService.getItem(this.KEY_LOCAL_STORAGE_BASE_CONFIG_V210_COLUMNS);
  ...
  prepareCsv(data: any, exportAllColumns: boolean, exportAllPages: boolean, currentPageSize: number): void {
    ...
    const columns = exportAllColumns ? this.columns.map(c => c.name) : this.displayedColumns;
    const headersToExport = columns.filter(columnName => columnName !== BaseConfigColumn.COLUMNS_MENU);
  ...
  initOpenedColumnMenuDialog(): void {
    this.columMenuComponent.keyLocalStorage = this.KEY_LOCAL_STORAGE_BASE_CONFIG_V210_COLUMNS;
  ...
  initOpenedConfigurationDialog(): void {
    this.configurationComponent.keyLocalStorage = this.KEY_LOCAL_STORAGE_BASE_CONFIG_V210_CONFIG;
