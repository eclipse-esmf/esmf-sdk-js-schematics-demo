...
import {FilterEnums, DefaultSortingFilterService} from './default-sorting.filter.service';
...
import {DefaultSortingDataSource} from './default-sorting-datasource';
 ...
 */
export enum DefaultSortingColumn {
    MOVING = 'moving',
...
@Component({
    selector: 'esmf-ui-default-sorting-v210',
    templateUrl: './default-sorting.component.html',
    styleUrls: ['./default-sorting.component.scss'],
})
export class DefaultSortingComponent implements OnInit, AfterViewInit, AfterViewChecked, OnDestroy {
    ...
    @ViewChild(DefaultSortingColumnMenuComponent) private columMenuComponent!: DefaultSortingColumnMenuComponent;
    @ViewChild(DefaultSortingConfigMenuComponent) private configurationComponent!: DefaultSortingConfigMenuComponent;
    ...
    readonly KEY_LOCAL_STORAGE_DEFAULT_SORTING_V210_COLUMNS = 'default_sorting_v210_columns';

    readonly KEY_LOCAL_STORAGE_DEFAULT_SORTING_V210_CONFIG = 'default_sorting_v210_config';

    totalItems: number = 0;
    selection = new SelectionModel<any>(this.isMultipleSelectionEnabled, []);
    dataSource: DefaultSortingDataSource;
    columnToSort: {sortColumnName: string; sortDirection: SortDirection} = {sortColumnName: 'endDate', sortDirection: 'asc'};
    displayedColumns: Array<string> = Object.values(DefaultSortingColumn);
    ...
    initializeColumns(): void {
        const configStorage = this.storageService.getItem(this.KEY_LOCAL_STORAGE_DEFAULT_SORTING_V210_CONFIG);
        const columnStorage = this.storageService.getItem(this.KEY_LOCAL_STORAGE_DEFAULT_SORTING_V210_COLUMNS);
        ...
        this.displayedColumns.forEach((displayedColumn: string): void => {
            if (displayedColumn === DefaultSortingColumn['COLUMNS_MENU'] || this.columns.find(column => column.name === displayedColumn)) {
                return;
            }
    ...
    hideColumn(column: DefaultSortingColumn): void {
        this.displayedColumns = this.displayedColumns.filter(columnName => columnName !== column);
    }

    showColumn(column: DefaultSortingColumn, index: number): void {
    ...
    resetDisplayedColumns(): void {
        this.displayedColumns = Object.values(DefaultSortingColumn);
    }
    ...
    prepareCsv(data: any, exportAllColumns: boolean, exportAllPages: boolean, currentPageSize: number): void {
        ...
        const columns = exportAllColumns ? this.columns.map(c => c.name) : this.displayedColumns;
        const headersToExport = columns.filter(columnName => columnName !== DefaultSortingColumn.COLUMNS_MENU);
    ...
    initOpenedColumnMenuDialog(): void {
        this.columMenuComponent.keyLocalStorage = this.KEY_LOCAL_STORAGE_DEFAULT_SORTING_V210_COLUMNS;
        this.columMenuComponent.columnsDefault = [
            ...Object.values(DefaultSortingColumn)

                .filter(columnName => columnName !== DefaultSortingColumn['COLUMNS_MENU'])
                .map(columnName => {
    ...
    initOpenedConfigurationDialog(): void {
        this.configurationComponent.keyLocalStorage = this.KEY_LOCAL_STORAGE_DEFAULT_SORTING_V210_CONFIG;
    ...
    setDisplayedColumns(columns: Array<Column>): void {
        ...
        if (DefaultSortingColumn['COLUMNS_MENU'] && columns[columns.length - 1].name !== DefaultSortingColumn['COLUMNS_MENU']) {
            displayedColumnsTmp.push({name: DefaultSortingColumn['COLUMNS_MENU'], selected: true});
        }
