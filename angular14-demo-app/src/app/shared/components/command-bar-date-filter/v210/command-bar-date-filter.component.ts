...
import {CommandBarDateFilterFilterService, FilterEnums} from './command-bar-date-filter.filter.service';
...
@Component({
  selector: 'esmf-ui-command-bar-date-filter-v210',
  templateUrl: './command-bar-date-filter.component.html',
  styleUrls: ['./command-bar-date-filter.component.scss'],
})
export class CommandBarDateFilterComponent implements OnInit, AfterViewInit, AfterViewChecked, OnDestroy {
  ...
  @ViewChild(CommandBarDateFilterConfigMenuComponent) private configurationComponent!: CommandBarDateFilterConfigMenuComponent;
  ...
  readonly KEY_LOCAL_STORAGE_COMMAND_BAR_DATE_FILTER_V210_COLUMNS = 'command_bar_date_filter_v210_columns';

  readonly KEY_LOCAL_STORAGE_COMMAND_BAR_DATE_FILTER_V210_CONFIG = 'command_bar_date_filter_v210_config';
  ...
  initializeColumns(): void {
    const configStorage = this.storageService.getItem(this.KEY_LOCAL_STORAGE_COMMAND_BAR_DATE_FILTER_V210_CONFIG);
    const columnStorage = this.storageService.getItem(this.KEY_LOCAL_STORAGE_COMMAND_BAR_DATE_FILTER_V210_COLUMNS);
    ...
    this.displayedColumns.forEach((displayedColumn: string): void => {
      if (displayedColumn === CommandBarDateFilterColumn['COLUMNS_MENU'] || this.columns.find(column => column.name === displayedColumn)) {
        return;
      }
  ...
  initOpenedColumnMenuDialog(): void {
    this.columMenuComponent.keyLocalStorage = this.KEY_LOCAL_STORAGE_COMMAND_BAR_DATE_FILTER_V210_COLUMNS;
  ...
  initOpenedConfigurationDialog(): void {
    this.configurationComponent.keyLocalStorage = this.KEY_LOCAL_STORAGE_COMMAND_BAR_DATE_FILTER_V210_CONFIG;
