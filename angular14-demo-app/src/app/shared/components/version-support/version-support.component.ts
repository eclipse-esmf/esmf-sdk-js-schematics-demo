...
@Component({
  selector: 'esmf-ui-version-support',
...
export class VersionSupportComponent implements OnInit, AfterViewInit, AfterViewChecked, OnChanges, OnDestroy {
  ...
  readonly KEY_LOCAL_STORAGE_VERSION_SUPPORT_COLUMNS = 'version_support_columns';

  readonly KEY_LOCAL_STORAGE_VERSION_SUPPORT_CONFIG = 'version_support_config';
  ...
  initializeColumns(): void {
    const configStorage = this.storageService.getItem(this.KEY_LOCAL_STORAGE_VERSION_SUPPORT_CONFIG);
    const columnStorage = this.storageService.getItem(this.KEY_LOCAL_STORAGE_VERSION_SUPPORT_COLUMNS);
  ...
  hideColumn(column: VersionSupportColumn): void {
    this.displayedColumns = this.displayedColumns.filter(columnName => columnName !== column);
  }

  showColumn(column: VersionSupportColumn, index: number): void {
  ...
  resetDisplayedColumns(): void {
    this.displayedColumns = Object.values(VersionSupportColumn);
  }
  ...
  initOpenedColumnMenuDialog(): void {
    this.columMenuComponent.keyLocalStorage = this.KEY_LOCAL_STORAGE_VERSION_SUPPORT_COLUMNS;
  ...
  initOpenedConfigurationDialog(): void {
    this.configurationComponent.keyLocalStorage = this.KEY_LOCAL_STORAGE_VERSION_SUPPORT_CONFIG;
  ...
  setDisplayedColumns(columns: Array<Column>): void {
    ...
    if (VersionSupportColumn['COLUMNS_MENU'] && columns[columns.length - 1].name !== VersionSupportColumn['COLUMNS_MENU']) {
      displayedColumnsTmp.push({name: VersionSupportColumn['COLUMNS_MENU'], selected: true});
    }
