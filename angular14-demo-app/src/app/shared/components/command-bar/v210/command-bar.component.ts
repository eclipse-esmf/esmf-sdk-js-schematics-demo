...
import {FilterEnums, CommandBarFilterService} from './command-bar.filter.service';
...
import {CommandBarConfigMenuComponent} from './command-bar-config-menu.component';
import {debounceTime, filter, map, takeUntil} from 'rxjs/operators';
import {Subject} from 'rxjs';
import {CustomCommandBarService} from './custom-command-bar.service';
import {MovementResponse} from './command-bar.service';
import {
    AbstractArrayNode,
    AbstractLogicalNode,
    AbstractNode,
...
export class CommandBarComponent implements OnInit, AfterViewInit, AfterViewChecked, OnDestroy {
    ...
    @ViewChild(CommandBarConfigMenuComponent) private configurationComponent!: CommandBarConfigMenuComponent;
    ...
    readonly KEY_LOCAL_STORAGE_COMMAND_BAR_V210_COLUMNS = 'command_bar_v210_columns';

    readonly KEY_LOCAL_STORAGE_COMMAND_BAR_V210_CONFIG = 'command_bar_v210_config';
    ...
    constructor(
        ...
        public filterService: CommandBarFilterService,
        private customcommandBarService: CustomCommandBarService,
        private cd: ChangeDetectorRef
    ) {
        this.dataSource = new CommandBarDataSource();
    ...
    initializeColumns(): void {
        const configStorage = this.storageService.getItem(this.KEY_LOCAL_STORAGE_COMMAND_BAR_V210_CONFIG);
        const columnStorage = this.storageService.getItem(this.KEY_LOCAL_STORAGE_COMMAND_BAR_V210_COLUMNS);
    ...
    applyFilters(): void {
        ...
        try {
            this.customcommandBarService.requestData(this.remoteAPI, {query: rqlStringTemp}).subscribe(
    ...
    downloadCsv(csvArray: any): void {
        ...
        try {
            this.customcommandBarService.downloadCsv(csvArray);
    ...
    initOpenedColumnMenuDialog(): void {
        this.columMenuComponent.keyLocalStorage = this.KEY_LOCAL_STORAGE_COMMAND_BAR_V210_COLUMNS;
    ...
    initOpenedConfigurationDialog(): void {
        this.configurationComponent.keyLocalStorage = this.KEY_LOCAL_STORAGE_COMMAND_BAR_V210_CONFIG;
