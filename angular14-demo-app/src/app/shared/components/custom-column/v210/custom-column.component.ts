...
import {FilterEnums, CustomColumnFilterService} from './custom-column.filter.service';
...
export class CustomColumnComponent implements OnInit, AfterViewInit, AfterViewChecked, OnDestroy {
    ...
    @ViewChild(CustomColumnConfigMenuComponent) private configurationComponent!: CustomColumnConfigMenuComponent;
    ...
    readonly KEY_LOCAL_STORAGE_CUSTOM_COLUMN_V210_COLUMNS = 'custom_column_v210_columns';

    readonly KEY_LOCAL_STORAGE_CUSTOM_COLUMN_V210_CONFIG = 'custom_column_v210_config';
    ...
    initializeColumns(): void {
        const configStorage = this.storageService.getItem(this.KEY_LOCAL_STORAGE_CUSTOM_COLUMN_V210_CONFIG);
        const columnStorage = this.storageService.getItem(this.KEY_LOCAL_STORAGE_CUSTOM_COLUMN_V210_COLUMNS);
    ...
    prepareCsv(data: any, exportAllColumns: boolean, exportAllPages: boolean, currentPageSize: number): void {
        ...
        const columns = exportAllColumns ? this.columns.map(c => c.name) : this.displayedColumns;
        const headersToExport = columns.filter(columnName => columnName !== CustomColumnColumn.COLUMNS_MENU);
    ...
    initOpenedColumnMenuDialog(): void {
        this.columMenuComponent.keyLocalStorage = this.KEY_LOCAL_STORAGE_CUSTOM_COLUMN_V210_COLUMNS;
    ...
    initOpenedConfigurationDialog(): void {
        this.configurationComponent.keyLocalStorage = this.KEY_LOCAL_STORAGE_CUSTOM_COLUMN_V210_CONFIG;
