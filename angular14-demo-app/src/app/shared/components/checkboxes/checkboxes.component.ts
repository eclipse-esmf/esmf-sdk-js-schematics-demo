...
import {CheckboxesService, MovementResponse} from './checkboxes.service';
...
export class CheckboxesComponent implements OnInit, AfterViewInit, AfterViewChecked, OnChanges {
    ...
    readonly KEY_LOCAL_STORAGE_CHECKBOXES_COLUMNS = 'checkboxes_columns';
    ...
    initializeColumns(): void {
        const columnStorage = this.storageService.getItem(this.KEY_LOCAL_STORAGE_CHECKBOXES_COLUMNS);
        ...
        this.displayedColumns.forEach((displayedColumn: string): void => {
            if (
                displayedColumn === CheckboxesColumn['CHECKBOX'] ||
                displayedColumn === CheckboxesColumn['COLUMNS_MENU'] ||
    ...
    initOpenedColumnMenuDialog(): void {
        this.columMenuComponent.keyLocalStorage = this.KEY_LOCAL_STORAGE_CHECKBOXES_COLUMNS;
    ...
    setDisplayedColumns(columns: Array<Column>): void {
        ...
        if (columns[0].name !== CheckboxesColumn['CHECKBOX']) {
            displayedColumnsTmp.push({name: CheckboxesColumn['CHECKBOX'], selected: true});
        }
