...
import {CheckboxesFilterService, FilterEnums} from './checkboxes.filter.service';
...
@Component({
  selector: 'esmf-ui-checkboxes-v210',
  templateUrl: './checkboxes.component.html',
  styleUrls: ['./checkboxes.component.scss'],
})
export class CheckboxesComponent implements OnInit, AfterViewInit, AfterViewChecked, OnDestroy {
  ...
  @ViewChild(CheckboxesColumnMenuComponent) private columMenuComponent!: CheckboxesColumnMenuComponent;
  @ViewChild(CheckboxesConfigMenuComponent) private configurationComponent!: CheckboxesConfigMenuComponent;
  ...
  readonly KEY_LOCAL_STORAGE_CHECKBOXES_V210_COLUMNS = 'checkboxes_v210_columns';

  readonly KEY_LOCAL_STORAGE_CHECKBOXES_V210_CONFIG = 'checkboxes_v210_config';
  ...
  initializeColumns(): void {
    const configStorage = this.storageService.getItem(this.KEY_LOCAL_STORAGE_CHECKBOXES_V210_CONFIG);
    const columnStorage = this.storageService.getItem(this.KEY_LOCAL_STORAGE_CHECKBOXES_V210_COLUMNS);
    ...
    this.displayedColumns.forEach((displayedColumn: string): void => {
      if (displayedColumn === CheckboxesColumn['COLUMNS_MENU'] || this.columns.find(column => column.name === displayedColumn)) {
        return;
      }
  ...
  hideColumn(column: CheckboxesColumn): void {
    this.displayedColumns = this.displayedColumns.filter(columnName => columnName !== column);
  }

  showColumn(column: CheckboxesColumn, index: number): void {
  ...
  resetDisplayedColumns(): void {
    this.displayedColumns = Object.values(CheckboxesColumn);
  }
  ...
  prepareCsv(data: any, exportAllColumns: boolean, exportAllPages: boolean, currentPageSize: number): void {
    ...
    const columns = exportAllColumns ? this.columns.map(c => c.name) : this.displayedColumns;
    const headersToExport = columns.filter(columnName => columnName !== CheckboxesColumn.COLUMNS_MENU);
  ...
  initOpenedColumnMenuDialog(): void {
    this.columMenuComponent.keyLocalStorage = this.KEY_LOCAL_STORAGE_CHECKBOXES_V210_COLUMNS;
    this.columMenuComponent.columnsDefault = [
      ...Object.values(CheckboxesColumn)
  ...
  initOpenedConfigurationDialog(): void {
    this.configurationComponent.keyLocalStorage = this.KEY_LOCAL_STORAGE_CHECKBOXES_V210_CONFIG;
  ...
  setDisplayedColumns(columns: Array<Column>): void {
    ...
    if (CheckboxesColumn['COLUMNS_MENU'] && columns[columns.length - 1].name !== CheckboxesColumn['COLUMNS_MENU']) {
      displayedColumnsTmp.push({name: CheckboxesColumn['COLUMNS_MENU'], selected: true});
    }
