...
import {ExcludedPropertyFilterService, FilterEnums} from './excluded-property.filter.service';
...
import {CustomExcludedPropertyService} from './custom-excluded-property.service';
import {ExcludedPropertyColumnMenuComponent} from './excluded-property-column-menu.component';
import {ExcludedPropertyConfigMenuComponent} from './excluded-property-config-menu.component';
import {MovementResponse} from './excluded-property.service';
 ...
 */
export enum ExcludedPropertyColumn {
  MOVING = 'moving',
...
@Component({
  selector: 'esmf-ui-excluded-property-v210',
  templateUrl: './excluded-property.component.html',
  styleUrls: ['./excluded-property.component.scss'],
})
export class ExcludedPropertyComponent implements OnInit, AfterViewInit, AfterViewChecked, OnDestroy {
  ...
  @ViewChild(ExcludedPropertyColumnMenuComponent) private columMenuComponent!: ExcludedPropertyColumnMenuComponent;
  @ViewChild(ExcludedPropertyConfigMenuComponent) private configurationComponent!: ExcludedPropertyConfigMenuComponent;
  ...
  readonly KEY_LOCAL_STORAGE_EXCLUDED_PROPERTY_V210_COLUMNS = 'excluded_property_v210_columns';

  readonly KEY_LOCAL_STORAGE_EXCLUDED_PROPERTY_V210_CONFIG = 'excluded_property_v210_config';
  ...
  constructor(
    ...
    public filterService: ExcludedPropertyFilterService,
    private customexcludedPropertyService: CustomExcludedPropertyService,
    private cd: ChangeDetectorRef
  ) {
    this.dataSource = new ExcludedPropertyDataSource();
  ...
  initializeColumns(): void {
    const configStorage = this.storageService.getItem(this.KEY_LOCAL_STORAGE_EXCLUDED_PROPERTY_V210_CONFIG);
    const columnStorage = this.storageService.getItem(this.KEY_LOCAL_STORAGE_EXCLUDED_PROPERTY_V210_COLUMNS);
    ...
    this.displayedColumns.forEach((displayedColumn: string): void => {
      if (displayedColumn === ExcludedPropertyColumn['COLUMNS_MENU'] || this.columns.find(column => column.name === displayedColumn)) {
        return;
      }
  ...
  hideColumn(column: ExcludedPropertyColumn): void {
    this.displayedColumns = this.displayedColumns.filter(columnName => columnName !== column);
  }

  showColumn(column: ExcludedPropertyColumn, index: number): void {
  ...
  resetDisplayedColumns(): void {
    this.displayedColumns = Object.values(ExcludedPropertyColumn);
  }
  ...
  applyFilters(): void {
    ...
    try {
      this.customexcludedPropertyService.requestData(this.remoteAPI, {query: rqlStringTemp}).subscribe(
  ...
  downloadCsv(csvArray: any): void {
    ...
    try {
      this.customexcludedPropertyService.downloadCsv(csvArray);
  ...
  initOpenedColumnMenuDialog(): void {
    this.columMenuComponent.keyLocalStorage = this.KEY_LOCAL_STORAGE_EXCLUDED_PROPERTY_V210_COLUMNS;
  ...
  initOpenedConfigurationDialog(): void {
    this.configurationComponent.keyLocalStorage = this.KEY_LOCAL_STORAGE_EXCLUDED_PROPERTY_V210_CONFIG;
  ...
  setDisplayedColumns(columns: Array<Column>): void {
    ...
    if (ExcludedPropertyColumn['COLUMNS_MENU'] && columns[columns.length - 1].name !== ExcludedPropertyColumn['COLUMNS_MENU']) {
      displayedColumnsTmp.push({name: ExcludedPropertyColumn['COLUMNS_MENU'], selected: true});
    }
