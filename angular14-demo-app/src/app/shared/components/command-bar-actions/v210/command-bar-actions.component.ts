...
import {FilterEnums, CommandBarActionsFilterService} from './command-bar-actions.filter.service';
...
@Component({
    selector: 'esmf-ui-command-bar-actions-v210',
    templateUrl: './command-bar-actions.component.html',
    styleUrls: ['./command-bar-actions.component.scss'],
})
export class CommandBarActionsComponent implements OnInit, AfterViewInit, AfterViewChecked, OnDestroy {
    ...
    @ViewChild(CommandBarActionsConfigMenuComponent) private configurationComponent!: CommandBarActionsConfigMenuComponent;
    ...
    readonly KEY_LOCAL_STORAGE_COMMAND_BAR_ACTIONS_V210_COLUMNS = 'command_bar_actions_v210_columns';

    readonly KEY_LOCAL_STORAGE_COMMAND_BAR_ACTIONS_V210_CONFIG = 'command_bar_actions_v210_config';
    ...
    initializeColumns(): void {
        const configStorage = this.storageService.getItem(this.KEY_LOCAL_STORAGE_COMMAND_BAR_ACTIONS_V210_CONFIG);
        const columnStorage = this.storageService.getItem(this.KEY_LOCAL_STORAGE_COMMAND_BAR_ACTIONS_V210_COLUMNS);
        ...
        this.displayedColumns.forEach((displayedColumn: string): void => {
            if (
                displayedColumn === CommandBarActionsColumn['COLUMNS_MENU'] ||
    ...
    initOpenedColumnMenuDialog(): void {
        this.columMenuComponent.keyLocalStorage = this.KEY_LOCAL_STORAGE_COMMAND_BAR_ACTIONS_V210_COLUMNS;
    ...
    initOpenedConfigurationDialog(): void {
        this.configurationComponent.keyLocalStorage = this.KEY_LOCAL_STORAGE_COMMAND_BAR_ACTIONS_V210_CONFIG;
