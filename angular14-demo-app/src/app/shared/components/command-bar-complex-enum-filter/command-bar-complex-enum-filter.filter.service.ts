...
import {Movement, Movement, WarningLevel, ResultState} from '../../types/movement/movement.types';
    ...
    applyEnumFilter(data: Array<Movement>) {
        ...
        filteredData =
            this.resultStateSelected.length === 0
                ? filteredData
                : filteredData.filter((item: Movement): boolean =>
                      this.resultStateSelected.includes(item.resultState.resultStateAttributeValue)
                  );
