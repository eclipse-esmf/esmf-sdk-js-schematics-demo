...
export class CommandBarSearchTableComponent implements OnInit, AfterViewInit, AfterViewChecked, OnChanges, OnDestroy {
  ...
  initializeColumns(): void {
    ...
    this.displayedColumns.forEach((displayedColumn: string): void => {
      if (displayedColumn === CommandBarSearchTableColumn['COLUMNS_MENU'] || this.columns.find(column => column.name === displayedColumn)) {
        return;
      }
  ...
  hideColumn(column: CommandBarSearchTableColumn): void {
    this.displayedColumns = this.displayedColumns.filter(columnName => columnName !== column);
  }

  showColumn(column: CommandBarSearchTableColumn, index: number): void {
  ...
  resetDisplayedColumns(): void {
    this.displayedColumns = Object.values(CommandBarSearchTableColumn);
  }
  ...
  prepareCsv(data: any, exportAllColumns: boolean, exportAllPages: boolean, currentPageSize: number): void {
    ...
    const headersToExport = columns.filter(columnName => columnName !== CommandBarSearchTableColumn.COLUMNS_MENU);
  ...
  initOpenedColumnMenuDialog(): void {
    ...
    this.columMenuComponent.columnsDefault = [
      ...Object.values(CommandBarSearchTableColumn)
        ...
        .filter(columnName => columnName !== CommandBarSearchTableColumn['COLUMNS_MENU'])
  ...
  setDisplayedColumns(columns: Array<Column>): void {
    ...
    if (CommandBarSearchTableColumn['COLUMNS_MENU'] && columns[columns.length - 1].name !== CommandBarSearchTableColumn['COLUMNS_MENU']) {
