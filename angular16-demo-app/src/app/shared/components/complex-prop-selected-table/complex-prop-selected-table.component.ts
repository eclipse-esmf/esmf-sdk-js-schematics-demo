...
export class ComplexPropSelectedTableComponent implements OnInit, AfterViewInit, AfterViewChecked, OnChanges {
  ...
  initializeColumns(): void {
    ...
    this.displayedColumns.forEach((displayedColumn: string): void => {
      if (
        displayedColumn === ComplexPropSelectedTableColumn['COLUMNS_MENU'] ||
  ...
  hideColumn(column: ComplexPropSelectedTableColumn): void {
    this.displayedColumns = this.displayedColumns.filter(columnName => columnName !== column);
  }

  showColumn(column: ComplexPropSelectedTableColumn, index: number): void {
  ...
  resetDisplayedColumns(): void {
    this.displayedColumns = Object.values(ComplexPropSelectedTableColumn);
  }
  ...
  initOpenedColumnMenuDialog(): void {
    ...
    this.columMenuComponent.columnsDefault = [
      ...Object.values(ComplexPropSelectedTableColumn)
        ...
        .filter(columnName => columnName !== ComplexPropSelectedTableColumn['COLUMNS_MENU'])
  ...
  setDisplayedColumns(columns: Array<Column>): void {
    ...
    if (
      ComplexPropSelectedTableColumn['COLUMNS_MENU'] &&
      columns[columns.length - 1].name !== ComplexPropSelectedTableColumn['COLUMNS_MENU']
    ) {
