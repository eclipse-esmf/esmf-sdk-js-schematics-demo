...
import {DefaultSortingTableDataSource} from './default-sorting-table-datasource';

import {SelectionModel} from '@angular/cdk/collections';
import {DomSanitizer} from '@angular/platform-browser';
import {TranslateService} from '@ngx-translate/core';
import {JSSdkLocalStorageService} from '../../services/storage.service';
import {DefaultSortingTableColumnMenuComponent} from './default-sorting-table-column-menu.component';

import {DefaultSortingTableService} from './default-sorting-table.service';
 ...
 */
export enum DefaultSortingTableColumn {
  MOVING = 'moving',
...
@Component({
  selector: 'esmf-ui-default-sorting-table',
  templateUrl: './default-sorting-table.component.html',
  styleUrls: ['./default-sorting-table.component.scss'],

  encapsulation: ViewEncapsulation.None,
})
export class DefaultSortingTableComponent implements OnInit, AfterViewInit, AfterViewChecked, OnChanges {
  ...
  @ViewChild(DefaultSortingTableColumnMenuComponent) private columMenuComponent!: DefaultSortingTableColumnMenuComponent;
  ...
  readonly KEY_LOCAL_STORAGE_DEFAULT_SORTING_TABLE_COLUMNS = 'default_sorting_table_columns';

  totalItems: number = 0;
  selection = new SelectionModel<any>(this.isMultipleSelectionEnabled, []);
  dataSource: DefaultSortingTableDataSource;

  columnToSort: {sortColumnName: string; sortDirection: SortDirection} = {sortColumnName: 'moving', sortDirection: 'asc'};
  displayedColumns: Array<string> = Object.values(DefaultSortingTableColumn);
  ...
  constructor(
    ...
    private defaultSortingTableService: DefaultSortingTableService
  ) {
    this.dataSource = new DefaultSortingTableDataSource(this.translateService);
  ...
  initializeColumns(): void {
    const columnStorage = this.storageService.getItem(this.KEY_LOCAL_STORAGE_DEFAULT_SORTING_TABLE_COLUMNS);
    ...
    this.displayedColumns.forEach((displayedColumn: string): void => {
      if (displayedColumn === DefaultSortingTableColumn['COLUMNS_MENU'] || this.columns.find(column => column.name === displayedColumn)) {
        return;
      }
  ...
  hideColumn(column: DefaultSortingTableColumn): void {
    this.displayedColumns = this.displayedColumns.filter(columnName => columnName !== column);
  }

  showColumn(column: DefaultSortingTableColumn, index: number): void {
  ...
  resetDisplayedColumns(): void {
    this.displayedColumns = Object.values(DefaultSortingTableColumn);
  }
  ...
  initOpenedColumnMenuDialog(): void {
    this.columMenuComponent.keyLocalStorage = this.KEY_LOCAL_STORAGE_DEFAULT_SORTING_TABLE_COLUMNS;
    this.columMenuComponent.columnsDefault = [
      ...Object.values(DefaultSortingTableColumn)

        .filter(columnName => columnName !== DefaultSortingTableColumn['COLUMNS_MENU'])
  ...
  setDisplayedColumns(columns: Array<Column>): void {
    ...
    if (DefaultSortingTableColumn['COLUMNS_MENU'] && columns[columns.length - 1].name !== DefaultSortingTableColumn['COLUMNS_MENU']) {
      displayedColumnsTmp.push({name: DefaultSortingTableColumn['COLUMNS_MENU'], selected: true});
    }
