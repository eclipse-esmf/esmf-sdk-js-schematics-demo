...
export class CommandBarActionsTableComponent implements OnInit, AfterViewInit, AfterViewChecked, OnChanges {
  ...
  displayedColumns: Array<string> = Object.values(CommandBarActionsTableColumn);
  ...
  initializeColumns(): void {
    ...
    this.displayedColumns.forEach((displayedColumn: string): void => {
      if (
        displayedColumn === CommandBarActionsTableColumn['COLUMNS_MENU'] ||
  ...
  hideColumn(column: CommandBarActionsTableColumn): void {
    this.displayedColumns = this.displayedColumns.filter(columnName => columnName !== column);
  }

  showColumn(column: CommandBarActionsTableColumn, index: number): void {
  ...
  resetDisplayedColumns(): void {
    this.displayedColumns = Object.values(CommandBarActionsTableColumn);
  }
  ...
  prepareCsv(data: any, exportAllColumns: boolean, exportAllPages: boolean, currentPageSize: number): void {
    ...
    const headersToExport = columns.filter(columnName => columnName !== CommandBarActionsTableColumn.COLUMNS_MENU);
  ...
  initOpenedColumnMenuDialog(): void {
    ...
    this.columMenuComponent.columnsDefault = [
      ...Object.values(CommandBarActionsTableColumn)
        ...
        .filter(columnName => columnName !== CommandBarActionsTableColumn['COLUMNS_MENU'])
  ...
  setDisplayedColumns(columns: Array<Column>): void {
    ...
    if (CommandBarActionsTableColumn['COLUMNS_MENU'] && columns[columns.length - 1].name !== CommandBarActionsTableColumn['COLUMNS_MENU']) {
