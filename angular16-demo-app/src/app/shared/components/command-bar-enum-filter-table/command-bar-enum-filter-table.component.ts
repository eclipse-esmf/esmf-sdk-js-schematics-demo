...
export class CommandBarEnumFilterTableComponent implements OnInit, AfterViewInit, AfterViewChecked, OnChanges {
  ...
  initializeColumns(): void {
    ...
    this.displayedColumns.forEach((displayedColumn: string): void => {
      if (
        displayedColumn === CommandBarEnumFilterTableColumn['COLUMNS_MENU'] ||
  ...
  hideColumn(column: CommandBarEnumFilterTableColumn): void {
    this.displayedColumns = this.displayedColumns.filter(columnName => columnName !== column);
  }

  showColumn(column: CommandBarEnumFilterTableColumn, index: number): void {
  ...
  resetDisplayedColumns(): void {
    this.displayedColumns = Object.values(CommandBarEnumFilterTableColumn);
  }
  ...
  prepareCsv(data: any, exportAllColumns: boolean, exportAllPages: boolean, currentPageSize: number): void {
    ...
    const headersToExport = columns.filter(columnName => columnName !== CommandBarEnumFilterTableColumn.COLUMNS_MENU);
  ...
  initOpenedColumnMenuDialog(): void {
    ...
    this.columMenuComponent.columnsDefault = [
      ...Object.values(CommandBarEnumFilterTableColumn)
        ...
        .filter(columnName => columnName !== CommandBarEnumFilterTableColumn['COLUMNS_MENU'])
  ...
  setDisplayedColumns(columns: Array<Column>): void {
    ...
    if (
      CommandBarEnumFilterTableColumn['COLUMNS_MENU'] &&
      columns[columns.length - 1].name !== CommandBarEnumFilterTableColumn['COLUMNS_MENU']
    ) {
