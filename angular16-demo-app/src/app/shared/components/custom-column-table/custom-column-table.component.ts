...
export class CustomColumnTableComponent implements OnInit, AfterViewInit, AfterViewChecked, OnChanges {
  ...
  initializeColumns(): void {
    ...
    this.displayedColumns.forEach((displayedColumn: string): void => {
      if (displayedColumn === CustomColumnTableColumn['COLUMNS_MENU'] || this.columns.find(column => column.name === displayedColumn)) {
        return;
      }
  ...
  hideColumn(column: CustomColumnTableColumn): void {
    this.displayedColumns = this.displayedColumns.filter(columnName => columnName !== column);
  }

  showColumn(column: CustomColumnTableColumn, index: number): void {
  ...
  resetDisplayedColumns(): void {
    this.displayedColumns = Object.values(CustomColumnTableColumn);
  }
  ...
  initOpenedColumnMenuDialog(): void {
    ...
    this.columMenuComponent.columnsDefault = [
      ...Object.values(CustomColumnTableColumn)
        ...
        .filter(columnName => columnName !== CustomColumnTableColumn['COLUMNS_MENU'])
  ...
  setDisplayedColumns(columns: Array<Column>): void {
    ...
    if (CustomColumnTableColumn['COLUMNS_MENU'] && columns[columns.length - 1].name !== CustomColumnTableColumn['COLUMNS_MENU']) {
