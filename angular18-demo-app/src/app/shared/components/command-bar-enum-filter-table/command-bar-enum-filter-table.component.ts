...
import {CommandBarEnumFilterTableFilterService} from './command-bar-enum-filter-table-filter.service';
...
import {CommandBarEnumFilterTableDataSource} from './command-bar-enum-filter-table-datasource';
...
import {CommandBarEnumFilterTableService} from './command-bar-enum-filter-table.service';
...
export const NON_DATA_COLUMNS: CommandBarEnumFilterTableColumn[] = [CommandBarEnumFilterTableColumn.COLUMNS_MENU];

@Component({
  selector: 'esmf-ui-command-bar-enum-filter-table',
  templateUrl: './command-bar-enum-filter-table.component.html',
  styleUrls: ['./command-bar-enum-filter-table.component.scss'],

  encapsulation: ViewEncapsulation.None,
})
export class CommandBarEnumFilterTableComponent implements OnInit, AfterViewInit, AfterViewChecked, OnChanges {
  ...
  @Input() hideColumns: CommandBarEnumFilterTableColumn[] = [];
  ...
  @ViewChild(CommandBarEnumFilterTableColumnMenuComponent) private columMenuComponent!: CommandBarEnumFilterTableColumnMenuComponent;
  ...
  readonly KEY_LOCAL_STORAGE_COMMAND_BAR_ENUM_FILTER_TABLE_COLUMNS = 'command_bar_enum_filter_table_columns';
  ...
  displayedColumns: CommandBarEnumFilterTableColumn[] = [];
  ...
  constructor(
    ...
    public filterService: CommandBarEnumFilterTableFilterService,
    private commandBarEnumFilterTableService: CommandBarEnumFilterTableService
  ) {
    this.dataSource = new CommandBarEnumFilterTableDataSource(this.translateService);
  ...
  initializeColumns(): void {
    this.columns = [];
    this.displayedColumns = Object.values(CommandBarEnumFilterTableColumn).filter(column => !this.hideColumns.includes(column));

    const columnStorage = this.storageService.getItem(this.KEY_LOCAL_STORAGE_COMMAND_BAR_ENUM_FILTER_TABLE_COLUMNS);

    if (columnStorage?.length > 0) {
      columnStorage
        .filter(
          (column: Column) =>
            !this.hideColumns.includes(column.name as CommandBarEnumFilterTableColumn) &&
    ...
    this.displayedColumns.forEach((displayedColumn: string): void => {
      if (
        !this.isAvailableDataColumn(displayedColumn as CommandBarEnumFilterTableColumn) ||
  ...
  openExportConfirmationDialog() {
    ...
    dialogRef
      ...
      .subscribe((event: {action: Action; exportAllPages: boolean; exportAllColumns: boolean}): void => {
        ...
        this.prepareCsv(
          this.commandBarEnumFilterTableService.flatten(this.data),
  ...
  downloadCsv(csvArray: any): void {
    ...
    try {
      this.commandBarEnumFilterTableService.downloadCsv(csvArray);
  ...
  initOpenedColumnMenuDialog(): void {
    this.columMenuComponent.keyLocalStorage = this.KEY_LOCAL_STORAGE_COMMAND_BAR_ENUM_FILTER_TABLE_COLUMNS;
    this.columMenuComponent.columnsDefault = [
      ...Object.values(CommandBarEnumFilterTableColumn)
  ...
  setDisplayedColumns(columns: Array<Column>): void {
    ...
    displayedColumnsTmp.push({name: CommandBarEnumFilterTableColumn['COLUMNS_MENU'], selected: true});

    this.columns = [...columns];
    this.displayedColumns = displayedColumnsTmp
      .filter(column => column.selected)
      .map(column => column.name as CommandBarEnumFilterTableColumn);
  }
  ...
  private isAvailableDataColumn(column: CommandBarEnumFilterTableColumn): boolean {
