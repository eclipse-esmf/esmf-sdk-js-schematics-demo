...
import {CustomColumnTableDataSource} from './custom-column-table-datasource';
...
import {CustomColumnTableColumnMenuComponent} from './custom-column-table-column-menu.component';

import {CustomColumnTableService} from './custom-column-table.service';
...
export const NON_DATA_COLUMNS: CustomColumnTableColumn[] = [CustomColumnTableColumn.COLUMNS_MENU];

@Component({
  selector: 'esmf-ui-custom-column-table',
  templateUrl: './custom-column-table.component.html',
  styleUrls: ['./custom-column-table.component.scss'],

  encapsulation: ViewEncapsulation.None,
})
export class CustomColumnTableComponent implements OnInit, AfterViewInit, AfterViewChecked, OnChanges {
  ...
  @Input() hideColumns: CustomColumnTableColumn[] = [];
  ...
  @ViewChild(CustomColumnTableColumnMenuComponent) private columMenuComponent!: CustomColumnTableColumnMenuComponent;
  ...
  readonly KEY_LOCAL_STORAGE_CUSTOM_COLUMN_TABLE_COLUMNS = 'custom_column_table_columns';
  ...
  displayedColumns: CustomColumnTableColumn[] = [];
  ...
  constructor(
    ...
    private customColumnTableService: CustomColumnTableService
  ) {
    this.dataSource = new CustomColumnTableDataSource(this.translateService);
  ...
  initializeColumns(): void {
    this.columns = [];
    this.displayedColumns = Object.values(CustomColumnTableColumn).filter(column => !this.hideColumns.includes(column));

    const columnStorage = this.storageService.getItem(this.KEY_LOCAL_STORAGE_CUSTOM_COLUMN_TABLE_COLUMNS);

    if (columnStorage?.length > 0) {
      columnStorage
        .filter(
          (column: Column) =>
            !this.hideColumns.includes(column.name as CustomColumnTableColumn) &&
    ...
    this.displayedColumns.forEach((displayedColumn: string): void => {
      if (
        !this.isAvailableDataColumn(displayedColumn as CustomColumnTableColumn) ||
  ...
  initOpenedColumnMenuDialog(): void {
    this.columMenuComponent.keyLocalStorage = this.KEY_LOCAL_STORAGE_CUSTOM_COLUMN_TABLE_COLUMNS;
    this.columMenuComponent.columnsDefault = [
      ...Object.values(CustomColumnTableColumn)
  ...
  setDisplayedColumns(columns: Array<Column>): void {
    ...
    displayedColumnsTmp.push({name: CustomColumnTableColumn['COLUMNS_MENU'], selected: true});

    this.columns = [...columns];
    this.displayedColumns = displayedColumnsTmp.filter(column => column.selected).map(column => column.name as CustomColumnTableColumn);
  }
  ...
  private isAvailableDataColumn(column: CustomColumnTableColumn): boolean {
