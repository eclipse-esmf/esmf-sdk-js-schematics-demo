...
export const NON_DATA_COLUMNS: BaseConfigTableColumn[] = [BaseConfigTableColumn.COLUMNS_MENU];
...
export class BaseConfigTableComponent implements OnInit, AfterViewInit, AfterViewChecked, OnChanges {
  ...
  @Input() hideColumns: BaseConfigTableColumn[] = [];
  ...
  displayedColumns: BaseConfigTableColumn[] = [];
  ...
  initializeColumns(): void {
    this.columns = [];
    this.displayedColumns = Object.values(BaseConfigTableColumn).filter(column => !this.hideColumns.includes(column));
    ...
    if (columnStorage?.length > 0) {
      columnStorage
        .filter(
          (column: Column) =>
            !this.hideColumns.includes(column.name as BaseConfigTableColumn) &&
    ...
    this.displayedColumns.forEach((displayedColumn: string): void => {
      if (
        !this.isAvailableDataColumn(displayedColumn as BaseConfigTableColumn) ||
  ...
  setDisplayedColumns(columns: Array<Column>): void {
    ...
    this.displayedColumns = displayedColumnsTmp.filter(column => column.selected).map(column => column.name as BaseConfigTableColumn);
  }
  ...
  private isAvailableDataColumn(column: BaseConfigTableColumn): boolean {
