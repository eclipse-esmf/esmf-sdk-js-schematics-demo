...
export class CommandBarActionsCardService {
  constructor(protected http: HttpClient, private translateService: TranslocoService) {
    this.http
      .get(
        `${baseUrl}/assets/i18n/shared/components/command-bar-actions-card/${this.translateService.getActiveLang()}.command-bar-actions-card.translation.json`
      )
