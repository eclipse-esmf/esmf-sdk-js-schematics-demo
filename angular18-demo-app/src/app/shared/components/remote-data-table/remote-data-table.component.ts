...
export const NON_DATA_COLUMNS: RemoteDataTableColumn[] = [RemoteDataTableColumn.COLUMNS_MENU];
...
export class RemoteDataTableComponent implements OnInit, AfterViewInit, AfterViewChecked, OnChanges, OnDestroy {
  ...
  @Input() hideColumns: RemoteDataTableColumn[] = [];
  ...
  displayedColumns: RemoteDataTableColumn[] = [];
  ...
  initializeColumns(): void {
    this.columns = [];
    this.displayedColumns = Object.values(RemoteDataTableColumn).filter(column => !this.hideColumns.includes(column));
    ...
    if (columnStorage?.length > 0) {
      columnStorage
        .filter(
          (column: Column) =>
            !this.hideColumns.includes(column.name as RemoteDataTableColumn) &&
    ...
    this.displayedColumns.forEach((displayedColumn: string): void => {
      if (
        !this.isAvailableDataColumn(displayedColumn as RemoteDataTableColumn) ||
  ...
  setDisplayedColumns(columns: Array<Column>): void {
    ...
    this.displayedColumns = displayedColumnsTmp.filter(column => column.selected).map(column => column.name as RemoteDataTableColumn);
  }
  ...
  private isAvailableDataColumn(column: RemoteDataTableColumn): boolean {
