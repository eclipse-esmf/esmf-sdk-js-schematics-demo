...
import {CommandBarDateFilterCardCardValues} from './command-bar-date-filter-card.component';

@Component({
  selector: 'command-bar-date-filter-card-command-bar',
  templateUrl: './command-bar-date-filter-card-command-bar.component.html',
  styleUrls: ['./command-bar-date-filter-card-command-bar.component.scss'],
})
export class CommandBarDateFilterCardCommandBarComponent implements AfterViewInit {
  ...
  @Input() KEY_LOCAL_STORAGE_COMMAND_BAR_DATE_FILTER_CARD_CONFIG: string = '';
  ...
  constructor(public filterService: CommandBarDateFilterCardFilterService, public dialog: MatDialog) {
    this.filterService.sortedProperty = this.commandBarDateFilterCardCardValues[0];
  }
