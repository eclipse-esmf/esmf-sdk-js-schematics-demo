...
import {CommandBarDateFilterCardFilterService, SortCard} from './command-bar-date-filter-card-filter.service';
...
@Component({
  selector: 'esmf-ui-command-bar-date-filter-card',
  templateUrl: './command-bar-date-filter-card.component.html',
  styleUrls: ['./command-bar-date-filter-card.component.scss'],

  encapsulation: ViewEncapsulation.None,
})
export class CommandBarDateFilterCardComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    private commandBarDateFilterCardService: CommandBarDateFilterCardService,
    private filterService: CommandBarDateFilterCardFilterService,
  ...
  getContext(data: Movement) {
    return {
      commandBarDateFilterCardCardValues: this.commandBarDateFilterCardCardValues,
      $implicit: data,
  ...
  private exportToCsv(exportData: Array<Movement>) {
    const headersCSV = unparse({
      fields: this.commandBarDateFilterCardCardValues.map(columnName => {
    ...
    this.downloadCsv(`${headersCSV}${unparse(exportData, {header: false, columns: this.commandBarDateFilterCardCardValues})}`);
  }

  private downloadCsv(csvArray: string) {
    this.downloadEvent.emit({error: false, success: false, inProgress: true});
    try {
      this.commandBarDateFilterCardService.downloadCsv(csvArray);
