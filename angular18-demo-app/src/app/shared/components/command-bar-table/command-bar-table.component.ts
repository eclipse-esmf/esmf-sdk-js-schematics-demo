...
export const NON_DATA_COLUMNS: CommandBarTableColumn[] = [CommandBarTableColumn.COLUMNS_MENU];
...
export class CommandBarTableComponent implements OnInit, AfterViewInit, AfterViewChecked, OnChanges {
  ...
  @Input() hideColumns: CommandBarTableColumn[] = [];
  ...
  displayedColumns: CommandBarTableColumn[] = [];
  ...
  initializeColumns(): void {
    this.columns = [];
    this.displayedColumns = Object.values(CommandBarTableColumn).filter(column => !this.hideColumns.includes(column));
    ...
    if (columnStorage?.length > 0) {
      columnStorage
        .filter(
          (column: Column) =>
            !this.hideColumns.includes(column.name as CommandBarTableColumn) &&
    ...
    this.displayedColumns.forEach((displayedColumn: string): void => {
      if (
        !this.isAvailableDataColumn(displayedColumn as CommandBarTableColumn) ||
  ...
  setDisplayedColumns(columns: Array<Column>): void {
    ...
    this.displayedColumns = displayedColumnsTmp.filter(column => column.selected).map(column => column.name as CommandBarTableColumn);
  }
  ...
  private isAvailableDataColumn(column: CommandBarTableColumn): boolean {
