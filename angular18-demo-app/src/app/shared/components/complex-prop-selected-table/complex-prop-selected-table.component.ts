...
import {ComplexPropSelectedTableDataSource} from './complex-prop-selected-table-datasource';
...
import {ComplexPropSelectedTableColumnMenuComponent} from './complex-prop-selected-table-column-menu.component';

import {ComplexPropSelectedTableService} from './complex-prop-selected-table.service';
...
export const NON_DATA_COLUMNS: ComplexPropSelectedTableColumn[] = [ComplexPropSelectedTableColumn.COLUMNS_MENU];

@Component({
  selector: 'esmf-ui-complex-prop-selected-table',
  templateUrl: './complex-prop-selected-table.component.html',
  styleUrls: ['./complex-prop-selected-table.component.scss'],

  encapsulation: ViewEncapsulation.None,
})
export class ComplexPropSelectedTableComponent implements OnInit, AfterViewInit, AfterViewChecked, OnChanges {
  ...
  @Input() hideColumns: ComplexPropSelectedTableColumn[] = [];
  ...
  @ViewChild(ComplexPropSelectedTableColumnMenuComponent) private columMenuComponent!: ComplexPropSelectedTableColumnMenuComponent;
  ...
  readonly KEY_LOCAL_STORAGE_COMPLEX_PROP_SELECTED_TABLE_COLUMNS = 'complex_prop_selected_table_columns';
  ...
  displayedColumns: ComplexPropSelectedTableColumn[] = [];
  ...
  constructor(
    ...
    private complexPropSelectedTableService: ComplexPropSelectedTableService
  ) {
    this.dataSource = new ComplexPropSelectedTableDataSource(this.translateService);
  ...
  initializeColumns(): void {
    this.columns = [];
    this.displayedColumns = Object.values(ComplexPropSelectedTableColumn).filter(column => !this.hideColumns.includes(column));

    const columnStorage = this.storageService.getItem(this.KEY_LOCAL_STORAGE_COMPLEX_PROP_SELECTED_TABLE_COLUMNS);

    if (columnStorage?.length > 0) {
      columnStorage
        .filter(
          (column: Column) =>
            !this.hideColumns.includes(column.name as ComplexPropSelectedTableColumn) &&
    ...
    this.displayedColumns.forEach((displayedColumn: string): void => {
      if (
        !this.isAvailableDataColumn(displayedColumn as ComplexPropSelectedTableColumn) ||
  ...
  initOpenedColumnMenuDialog(): void {
    this.columMenuComponent.keyLocalStorage = this.KEY_LOCAL_STORAGE_COMPLEX_PROP_SELECTED_TABLE_COLUMNS;
    this.columMenuComponent.columnsDefault = [
      ...Object.values(ComplexPropSelectedTableColumn)
  ...
  setDisplayedColumns(columns: Array<Column>): void {
    ...
    displayedColumnsTmp.push({name: ComplexPropSelectedTableColumn['COLUMNS_MENU'], selected: true});

    this.columns = [...columns];
    this.displayedColumns = displayedColumnsTmp
      .filter(column => column.selected)
      .map(column => column.name as ComplexPropSelectedTableColumn);
  }
  ...
  private isAvailableDataColumn(column: ComplexPropSelectedTableColumn): boolean {
