...
export const NON_DATA_COLUMNS: CheckboxesTableColumn[] = [CheckboxesTableColumn.COLUMNS_MENU];
...
export class CheckboxesTableComponent implements OnInit, AfterViewInit, AfterViewChecked, OnChanges {
  ...
  @Input() hideColumns: CheckboxesTableColumn[] = [];
  ...
  displayedColumns: CheckboxesTableColumn[] = [];
  ...
  initializeColumns(): void {
    this.columns = [];
    this.displayedColumns = Object.values(CheckboxesTableColumn).filter(column => !this.hideColumns.includes(column));
    ...
    if (columnStorage?.length > 0) {
      columnStorage
        .filter(
          (column: Column) =>
            !this.hideColumns.includes(column.name as CheckboxesTableColumn) &&
    ...
    this.displayedColumns.forEach((displayedColumn: string): void => {
      if (
        ...
        !this.isAvailableDataColumn(displayedColumn as CheckboxesTableColumn) ||
  ...
  setDisplayedColumns(columns: Array<Column>): void {
    ...
    this.displayedColumns = displayedColumnsTmp.filter(column => column.selected).map(column => column.name as CheckboxesTableColumn);
  }
  ...
  private isAvailableDataColumn(column: CheckboxesTableColumn): boolean {
