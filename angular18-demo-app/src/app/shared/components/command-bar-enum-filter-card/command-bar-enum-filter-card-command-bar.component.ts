...
import {CommandBarEnumFilterCardCardValues} from './command-bar-enum-filter-card.component';

@Component({
  selector: 'command-bar-enum-filter-card-command-bar',
  templateUrl: './command-bar-enum-filter-card-command-bar.component.html',
  styleUrls: ['./command-bar-enum-filter-card-command-bar.component.scss'],
})
export class CommandBarEnumFilterCardCommandBarComponent implements AfterViewInit {
  ...
  @Input() KEY_LOCAL_STORAGE_COMMAND_BAR_ENUM_FILTER_CARD_CONFIG: string = '';
  ...
  constructor(public filterService: CommandBarEnumFilterCardFilterService, public dialog: MatDialog) {
    this.filterService.sortedProperty = this.commandBarEnumFilterCardCardValues[0];
  }
