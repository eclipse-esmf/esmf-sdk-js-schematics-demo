...
@Component({
  selector: 'command-bar-search-table-command-bar',
...
export class CommandBarSearchTableCommandBarComponent implements AfterViewInit {
  ...
  constructor(public filterService: CommandBarSearchTableFilterService, public dialog: MatDialog) {}
  ...
  openSettingsFromCollapsed() {
    const configMenuDialogRef = this.dialog.open(CommandBarSearchTableConfigMenuComponent, {
      data: {configs: this.configs, keyLocalStorage: this.KEY_LOCAL_STORAGE_COMMAND_BAR_SEARCH_TABLE_CONFIG},
    });
