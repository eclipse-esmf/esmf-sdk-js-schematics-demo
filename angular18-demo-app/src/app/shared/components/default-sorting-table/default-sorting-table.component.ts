...
export const NON_DATA_COLUMNS: DefaultSortingTableColumn[] = [DefaultSortingTableColumn.COLUMNS_MENU];
...
export class DefaultSortingTableComponent implements OnInit, AfterViewInit, AfterViewChecked, OnChanges {
  ...
  @Input() hideColumns: DefaultSortingTableColumn[] = [];
  ...
  displayedColumns: DefaultSortingTableColumn[] = [];
  ...
  initializeColumns(): void {
    this.columns = [];
    this.displayedColumns = Object.values(DefaultSortingTableColumn).filter(column => !this.hideColumns.includes(column));
    ...
    if (columnStorage?.length > 0) {
      columnStorage
        .filter(
          (column: Column) =>
            !this.hideColumns.includes(column.name as DefaultSortingTableColumn) &&
    ...
    this.displayedColumns.forEach((displayedColumn: string): void => {
      if (
        !this.isAvailableDataColumn(displayedColumn as DefaultSortingTableColumn) ||
  ...
  setDisplayedColumns(columns: Array<Column>): void {
    ...
    this.displayedColumns = displayedColumnsTmp.filter(column => column.selected).map(column => column.name as DefaultSortingTableColumn);
  }
  ...
  private isAvailableDataColumn(column: DefaultSortingTableColumn): boolean {
