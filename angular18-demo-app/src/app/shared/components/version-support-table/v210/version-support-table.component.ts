...
export const NON_DATA_COLUMNS: VersionSupportTableColumn[] = [VersionSupportTableColumn.COLUMNS_MENU];
...
export class VersionSupportTableComponent implements OnInit, AfterViewInit, AfterViewChecked, OnChanges, OnDestroy {
  ...
  @Input() hideColumns: VersionSupportTableColumn[] = [];
  ...
  displayedColumns: VersionSupportTableColumn[] = [];
  ...
  initializeColumns(): void {
    this.columns = [];
    this.displayedColumns = Object.values(VersionSupportTableColumn).filter(column => !this.hideColumns.includes(column));
    ...
    if (columnStorage?.length > 0) {
      columnStorage
        .filter(
          (column: Column) =>
            !this.hideColumns.includes(column.name as VersionSupportTableColumn) &&
    ...
    this.displayedColumns.forEach((displayedColumn: string): void => {
      if (
        ...
        !this.isAvailableDataColumn(displayedColumn as VersionSupportTableColumn) ||
  ...
  prepareCsv(data: any, exportAllColumns: boolean, exportAllPages: boolean, currentPageSize: number): void {
    ...
    const headersCSV = unparse({
      fields: headersToExport.map(columnName => {
        const translatedHeader = this.translateService.translate(`movement.v210.${columnName}.preferredName`);
  ...
  setDisplayedColumns(columns: Array<Column>): void {
    ...
    this.displayedColumns = displayedColumnsTmp.filter(column => column.selected).map(column => column.name as VersionSupportTableColumn);
  }
  ...
  private isAvailableDataColumn(column: VersionSupportTableColumn): boolean {
