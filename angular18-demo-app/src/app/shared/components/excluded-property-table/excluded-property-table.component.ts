...
export const NON_DATA_COLUMNS: ExcludedPropertyTableColumn[] = [ExcludedPropertyTableColumn.COLUMNS_MENU];
...
export class ExcludedPropertyTableComponent implements OnInit, AfterViewInit, AfterViewChecked, OnChanges {
  ...
  @Input() hideColumns: ExcludedPropertyTableColumn[] = [];
  ...
  displayedColumns: ExcludedPropertyTableColumn[] = [];
  ...
  initializeColumns(): void {
    this.columns = [];
    this.displayedColumns = Object.values(ExcludedPropertyTableColumn).filter(column => !this.hideColumns.includes(column));
    ...
    if (columnStorage?.length > 0) {
      columnStorage
        .filter(
          (column: Column) =>
            !this.hideColumns.includes(column.name as ExcludedPropertyTableColumn) &&
    ...
    this.displayedColumns.forEach((displayedColumn: string): void => {
      if (
        !this.isAvailableDataColumn(displayedColumn as ExcludedPropertyTableColumn) ||
  ...
  setDisplayedColumns(columns: Array<Column>): void {
    ...
    this.displayedColumns = displayedColumnsTmp.filter(column => column.selected).map(column => column.name as ExcludedPropertyTableColumn);
  }
  ...
  private isAvailableDataColumn(column: ExcludedPropertyTableColumn): boolean {
