...
export class RowActionButtonTableService {
  constructor(protected http: HttpClient, private translateService: TranslocoService) {
    this.http
      .get(
        `${baseUrl}/assets/i18n/shared/components/row-action-button-table/${this.translateService.getActiveLang()}.row-action-button-table.translation.json`
      )
