...
export class CommandBarSearchCardService {
  constructor(protected http: HttpClient, private translateService: TranslocoService) {
    this.http
      .get(
        `${baseUrl}/assets/i18n/shared/components/command-bar-search-card/${this.translateService.getActiveLang()}.command-bar-search-card.translation.json`
      )
