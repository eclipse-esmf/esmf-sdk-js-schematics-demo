...
export class CommandBarCardService {
  constructor(protected http: HttpClient, private translateService: TranslocoService) {
    this.http
      .get(
        `${baseUrl}/assets/i18n/shared/components/command-bar-card/${this.translateService.getActiveLang()}.command-bar-card.translation.json`
      )
