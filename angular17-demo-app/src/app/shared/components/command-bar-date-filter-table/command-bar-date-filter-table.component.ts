...
export class CommandBarDateFilterTableComponent implements OnInit, AfterViewInit, AfterViewChecked, OnChanges {
  ...
  initializeColumns(): void {
    ...
    this.displayedColumns.forEach((displayedColumn: string): void => {
      if (
        displayedColumn === CommandBarDateFilterTableColumn['COLUMNS_MENU'] ||
  ...
  hideColumn(column: CommandBarDateFilterTableColumn): void {
    this.displayedColumns = this.displayedColumns.filter(columnName => columnName !== column);
  }

  showColumn(column: CommandBarDateFilterTableColumn, index: number): void {
  ...
  resetDisplayedColumns(): void {
    this.displayedColumns = Object.values(CommandBarDateFilterTableColumn);
  }
  ...
  prepareCsv(data: any, exportAllColumns: boolean, exportAllPages: boolean, currentPageSize: number): void {
    ...
    const headersToExport = columns.filter(columnName => columnName !== CommandBarDateFilterTableColumn.COLUMNS_MENU);
  ...
  initOpenedColumnMenuDialog(): void {
    ...
    this.columMenuComponent.columnsDefault = [
      ...Object.values(CommandBarDateFilterTableColumn)
        ...
        .filter(columnName => columnName !== CommandBarDateFilterTableColumn['COLUMNS_MENU'])
  ...
  setDisplayedColumns(columns: Array<Column>): void {
    ...
    if (
      CommandBarDateFilterTableColumn['COLUMNS_MENU'] &&
      columns[columns.length - 1].name !== CommandBarDateFilterTableColumn['COLUMNS_MENU']
    ) {
