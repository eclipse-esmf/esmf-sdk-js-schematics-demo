...
export class CommandBarDateFilterTableService {
  constructor(protected http: HttpClient, private translateService: TranslocoService) {
    this.http
      .get(
        `${baseUrl}/assets/i18n/shared/components/command-bar-date-filter-table/${this.translateService.getActiveLang()}.command-bar-date-filter-table.translation.json`
      )
