...
@Component({
  selector: 'esmf-ui-command-bar-search-card',
  templateUrl: './command-bar-search-card.component.html',
  styleUrls: ['./command-bar-search-card.component.scss'],

  encapsulation: ViewEncapsulation.None,
})
export class CommandBarSearchCardComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(
    ...
    private commandBarSearchCardService: CommandBarSearchCardService,
    private filterService: CommandBarSearchCardFilterService,
  ...
  getContext(data: Movement) {
    return {
      commandBarSearchCardCardValues: this.commandBarSearchCardCardValues,
      $implicit: data,
  ...
  private exportToCsv(exportData: Array<Movement>) {
    const headersCSV = unparse({
      fields: this.commandBarSearchCardCardValues.map(columnName => {
    ...
    this.downloadCsv(`${headersCSV}${unparse(exportData, {header: false, columns: this.commandBarSearchCardCardValues})}`);
  }

  private downloadCsv(csvArray: string) {
    this.downloadEvent.emit({error: false, success: false, inProgress: true});
    try {
      this.commandBarSearchCardService.downloadCsv(csvArray);
