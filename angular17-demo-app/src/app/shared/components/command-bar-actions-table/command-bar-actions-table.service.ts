...
export class CommandBarActionsTableService {
  constructor(protected http: HttpClient, private translateService: TranslocoService) {
    this.http
      .get(
        `${baseUrl}/assets/i18n/shared/components/command-bar-actions-table/${this.translateService.getActiveLang()}.command-bar-actions-table.translation.json`
      )
