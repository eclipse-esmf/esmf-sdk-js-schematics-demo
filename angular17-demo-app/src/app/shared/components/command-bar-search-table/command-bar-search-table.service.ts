...
export class CommandBarSearchTableService {
  constructor(protected http: HttpClient, private translateService: TranslocoService) {
    this.http
      .get(
        `${baseUrl}/assets/i18n/shared/components/command-bar-search-table/${this.translateService.getActiveLang()}.command-bar-search-table.translation.json`
      )
