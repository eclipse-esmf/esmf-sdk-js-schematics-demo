...
export class CommandBarEnumFilterTableService {
  constructor(protected http: HttpClient, private translateService: TranslocoService) {
    this.http
      .get(
        `${baseUrl}/assets/i18n/shared/components/command-bar-enum-filter-table/${this.translateService.getActiveLang()}.command-bar-enum-filter-table.translation.json`
      )
