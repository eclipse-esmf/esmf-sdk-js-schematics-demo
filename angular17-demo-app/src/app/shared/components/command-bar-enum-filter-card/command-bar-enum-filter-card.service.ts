...
export class CommandBarEnumFilterCardService {
  constructor(protected http: HttpClient, private translateService: TranslocoService) {
    this.http
      .get(
        `${baseUrl}/assets/i18n/shared/components/command-bar-enum-filter-card/${this.translateService.getActiveLang()}.command-bar-enum-filter-card.translation.json`
      )
